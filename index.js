...
class EasyPdf {
    ...
    create (options, cb = () => {
        ...
        return new Promise((resolve, reject) => {
            const url = 'https://api.easypdf.io/v2/free/pdf';
    ...
    download (filename = 'sample.pdf', pdf = this._pdf) {
        if (filename === undefined || isBase64(filename)) {
            throw new Error('Invalid filename.');
        }

        if (typeof window === 'undefined') {
            throw new Error('Easy PDF download() is only supported in the browser.');
        } else {
    ...
    render (elementId, pdf = this._pdf, cb = () => {
        ...
        return new Promise((resolve) => {
            if (typeof window === 'undefined') {
                throw new Error('Easy PDF render() is only supported in the browser.');
            } else {
                this._elementId = elementId;
